...
import { getRunFinishDate } from './getRunFinishDate';
...
async function publishReport() {
  ...
  // Define the new report folder name, e.g. allure-report-2025-03-14-ios-2.8.7
  const baseReportDir = allureCurrentReportDir;
  const runFinishDate = await getRunFinishDate();
  ...
  const deployCommand = `gh-pages -d ${newReportDir} --dest reports/${publishedReportName} --message "Deploy report for ${platform} ${build}"`;
